...
for value in GITHUB_PATH GITHUB_REPOSITORY GITHUB_RUN_ID RELEASE_CANDIDATE_BUCKET RELEASE_CANDIDATE_PREFIX RELEASE_CANDIDATE_TRAIN_SHA256 RUNNER_TEMP; do
  if [[ -z "${!value:-}" ]]; then
...
destination="s3://${candidate_bucket}/${candidate_prefix}/${candidate_train_sha256}/${GITHUB_REPOSITORY}/${GITHUB_RUN_ID}/docs/${docs_version}/"
