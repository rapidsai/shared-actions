...
| Term | Meaning |
| ---- | ------- |
| **Artifact** | A file produced by a build job: a `.conda` file, a wheel, a JAR, and so on. |
...
| **SBOM** | Software bill of materials, which lists components by ecosystem, name and version. The [CycloneDX](https://cyclonedx.org/docs/1.6/json/) SBOM this action generates identifies the artifact and its digest only; it does not list its dependencies. Adding dependencies here will be a future project. |
...
will accept a producer-supplied dependency SBOM, which would use a different
`sbom_kind`.
...
that share a common `release_catalog_key` are versioned, validated, ordered, published,
