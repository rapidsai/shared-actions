...
of the job (repository, commit, workflow, run). See [`examples/cuvs-java`](examples/cuvs-java) for a complete
companion.
...
will accept a caller-supplied dependency SBOM; that will be a new
`sbom_kind` value and a new `schema_version`, so existing consumers can tell
the two apart.
